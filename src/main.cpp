...
int main(int argc, char *argv[]) {
  ...
  //  for(auto j = 0; j < 8; j++)
  //  {
  //    std::cout << triangles[i].vertices[0].v[0][j] << " , " << triangles[i].vertices[0].v[1][j] << " , " << triangles[i].vertices[0].v[2][j] << std::endl;
  //    std::cout << triangles[i].vertices[1].v[0][j] << " , " << triangles[i].vertices[1].v[1][j] << " , " << triangles[i].vertices[1].v[2][j] << std::endl;
  //    std::cout << triangles[i].vertices[2].v[0][j] << " , " << triangles[i].vertices[2].v[1][j] << " , " << triangles[i].vertices[2].v[2][j] << std::endl;
  //  }
  //}
  ...
  for (int h = image_height - 1; h >= 0; --h) {
    for (int w = 0; w < image_width; w++) {
      ...
      auto flag = false;
      int min_geomID, min_primID;
      float t, u, v;

      for(auto i = 0; i < SIMD_WIDTH; i++)
      {
        auto iter_size = i == triangles.size() - 1 ? triangles.size() % SIMD_WIDTH : SIMD_WIDTH;
        if(ispc::ispc_intersect(iter_size, triangles[i], ispc_ray))
        {
          exit(0);
          flag = true;
          t = std::numeric_limits<float>::max();
          for(auto j = 0; j < iter_size; j++)
          {
            if(t > ispc_ray.t[j] && ispc_ray.geomID[j] != -1 && ispc_ray.primID[j] != -1)
            {
              t = ispc_ray.t[j];
      ...
      //std::cout << "Geom ID: " << min_geomID << ";";
      //std::cout << "Prim ID: " << min_primID << ";";
